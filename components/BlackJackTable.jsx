...
export function BlackJackTable({
  ...
  const hit = () => {
    ...
    const newPack = utils.returnNewDeckOfCardsWithSpecificCardRemoved(
      card,
      deckOfCards,
    );
  ...
  useEffect(() => {
    ...
    if (isBust) {
      ...
      const results = utils.determineWinner(value, dealerCardValue, scores);
  ...
  useEffect(() => {
    ...
    if (gameStarted) {
      ...
      if (hitAgain) {
        ...
        const newCards = utils.returnNewDeckOfCardsWithSpecificCardRemoved(
          card,
          deckOfCards,
        );
  ...
  useEffect(() => {
    if (userStick) {
      const results = utils.determineWinner(
        userCardValue,
        dealerCardValue,
        scores,
      );
  ...
  return (
    <section className="table">
      <div className="content">
        ...
        <SectionTitle title="Dealer's Cards" gameStarted={gameStarted} />
        ...
        />
        <AppBar background="#344955" height="55px" {...appBarProps} />
